...
| Date | Upstream Version | Tag commit | Notes |
|------|-----------------|------------|-------|
| 2026-05-07 | v1.41.0 | `0ed360e6` | **Not merged.** 342 commits ahead as of 2026-05-15. Key features: six namespace meta-skills replacing 86-skill flat listing (cuts cold-start tokens from ~2150 → ~120); skill surface consolidated 86 → 59 (`commands/gsd/*.md` — 31 micro-skills absorbed into grouped parents `capture`, `phase`, `config`, `workspace` + flag variants); `/gsd-health --context` context utilization guard (60%/70% thresholds); `/gsd-edit-phase` command; post-merge build & test gate in execute-phase step 5.6; **`execute-phase/steps/per-plan-worktree-gate.md` — new upstream step enforcing per-plan worktree isolation (aligns with fork's `workflow.use_worktrees` and new sdlc.md §2 guidance)**; phase-lifecycle status-line read-side; workstream config inheritance; SDK runtime bridge seam (`feat/sdk-runtime-bridge-seam`, #3158); `discuss-phase` lazy file loading (13k → ~0 tokens at entry). **Conflict risk (HIGH):** `agents/gsd-executor.md`, `agents/gsd-planner.md`, `agents/gsd-verifier.md`, `get-shit-done/bin/lib/config-schema.cjs`, `workflows/discuss-phase.md`, `workflows/execute-phase.md`, `commands/gsd/settings.md`. Key fork preservation points: plan_bounce 12.5, Devil's Advocate 12.6, `is_fork`, statusline fork-awareness, TEXT_MODE, `questions_per_area`, `playwright_verification`, 21 fork config keys in `config-schema.cjs`. |
| 2026-05-15 | v1.42.1 | `d4d41786` | **Not merged** (incremental on v1.41.0). Agent-launch reasoning transport (#3483); `project_code` prefix now applied consistently in `/gsd-discuss-phase` and `/gsd-plan-phase` first-touch creation (#3287); `buildStateFrontmatter` now counts nested `plans/<N>-PLAN-<NN>-<slug>.md` files (#3261). **Conflict risk:** `agents/gsd-executor.md`, `agents/gsd-code-reviewer.md`, `agents/gsd-debugger.md`, `agents/gsd-phase-researcher.md`, `agents/gsd-verifier.md`, `agents/gsd-planner.md`, `get-shit-done/bin/lib/config-schema.cjs`. |

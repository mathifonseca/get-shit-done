...
describe('#1951 gsd-planner: one-way inserts a checkpoint, reversible does not', () => {
  ...
  test(`planner stays under the ${PLANNER_CAP}-char agent cap`, () => {
    const planner = read(PLANNER);
    assert.ok(
      planner.length < PLANNER_CAP,
      `gsd-planner.md is ${planner.length} chars, must be < ${PLANNER_CAP} (LF-normalized). `

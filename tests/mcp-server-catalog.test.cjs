...
describe('prompts — protocol surface', () => {
  test('lists commands as prompts by bare name (row 31)', () => {
    ...
    // Derive from disk instead of a hardcoded count: this fork ships 2 commands
    // upstream does not (sdlc-audit, teach-phase), and a literal here goes stale
    // on every command added on either side. The invariant is parity with
    // commands/gsd/*.md, not a specific number.
    const expectedPromptCount = fs
      .readdirSync(path.join(__dirname, '..', 'commands', 'gsd'))
      .filter((f) => f.endsWith('.md')).length;
    assert.equal(
      res.result.prompts.length,
      expectedPromptCount,
      `must list all ${expectedPromptCount} commands/gsd/*.md as prompts`,
    );

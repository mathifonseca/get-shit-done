...
//
// **Fork ratchet (SDLC-aligned).** The fork's verifier/executor/code-reviewer carry
// opinionated blocks upstream does not ship (verification_discipline, dead-code scan,
...
const XL_AGENTS = new Set([
  ...
  'gsd-verifier',
  // SDLC-aligned fork: gsd-executor carries the test-contract (tests are read-only),
  // Makefile-preference and Edge Case Hunter blocks on top of upstream's executor.
  'gsd-executor',
]);
...
const LARGE_AGENTS = new Set([
  ...
  'gsd-doc-writer',
  'gsd-plan-checker',
  // SDLC-aligned fork: gsd-code-reviewer carries the fork's adversarial-validation
  // and dead-code/context-pollution rubrics — promoted from DEFAULT.
  'gsd-code-reviewer',
  'gsd-code-fixer',
  'gsd-codebase-mapper',

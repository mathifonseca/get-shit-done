...
describe('progress counters correct after plan execution (#1589)', () => {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // `completed_plans` must never exceed `total_plans`
  //
  // `buildStateFrontmatter` summed RAW summary counts into `completed_plans`
  // while `total_plans` summed plan counts. Summaries legitimately outnumber
  // plans — a brief-driven gap closure ships a SUMMARY with no PLAN (the brief
  // IS the approved plan) — so any project with one produced
  // `completed_plans > total_plans`: frontmatter the `state-check` coherence
  // gate rejects, re-corrupted on every state mutation. Observed at 64 > 59 on
  // a phase holding 14 summaries against 9 plans.
  //
  // Each phase's contribution is now clamped to its own `planCount`, the same
  // asymmetry `scanPhasePlans` already models in `completed`
  // (`summaryCount >= planCount`).
  // ───────────────────────────────────────────────────────────────────────────

  /** Flat phase dir with `plans` PLAN files and `summaries` SUMMARY files. */
  function writePhaseFiles(dir, phaseNum, plans, summaries) {
    fs.mkdirSync(dir, { recursive: true });
    const pad = (i) => String(i).padStart(2, '0');
    for (let i = 1; i <= plans; i++) {
      fs.writeFileSync(path.join(dir, `${phaseNum}-${pad(i)}-PLAN.md`), '# Plan\n');
    }
    for (let i = 1; i <= summaries; i++) {
      fs.writeFileSync(path.join(dir, `${phaseNum}-${pad(i)}-SUMMARY.md`), '# Summary\n');
    }
  }

  function progressAfterWrite() {
    const upd = runGsdTools('state update Status "Executing"', tmpDir);
    assert.ok(upd.success, `state update failed: ${upd.error}`);
    const jsonResult = runGsdTools('state json', tmpDir);
    assert.ok(jsonResult.success, `state json failed: ${jsonResult.error}`);
  ...
  test('a phase with MORE summaries than plans cannot push completed_plans over total_plans', () => {
    ...
    writePhaseFiles(path.join(phases, '01-foundation'), '01', 2, 2);
    // Phase 02 carries 3 gap-closure summaries with no plan of their own.
    writePhaseFiles(path.join(phases, '02-api'), '02', 2, 5);
    ...
    const progress = progressAfterWrite();
    const total = Number(progress.total_plans);
    const completed = Number(progress.completed_plans);

    assert.strictEqual(total, 4, 'total_plans counts PLAN files (2 + 2)');
    assert.strictEqual(
      completed,
      4,
      'completed_plans clamps per phase: min(2,2) + min(5,2). Pre-fix this was 7 — the raw summary sum'
    );
    assert.ok(
      completed <= total,
      `completed_plans (${completed}) must never exceed total_plans (${total})`
    );
  });

  test('the clamp does not floor partial progress — 1 of 3 still reports 1', () => {
    // Discrimination arm: without this, a fix that simply returned total_plans
    // would satisfy the assertion above and teach nothing.
    writePhaseFiles(path.join(tmpDir, '.planning', 'phases', '01-foundation'), '01', 3, 1);
    fs.writeFileSync(
      path.join(tmpDir, '.planning', 'STATE.md'),
      '# Project State\n\n**Current Phase:** 01\n**Status:** Executing\n'
    );
    ...
    const progress = progressAfterWrite();
    assert.strictEqual(Number(progress.total_plans), 3);
    assert.strictEqual(
      Number(progress.completed_plans),
      1,
      'an incomplete phase still reports its real summary count'
    );
  });
});

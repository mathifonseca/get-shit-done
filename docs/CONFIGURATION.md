...
| `workflow.milestone_retro` | boolean | `false` | Enable multi-lens milestone retro. When `true`, spawns isolated lens subagents during `write_retrospective` to produce a **Tensions** section in `.planning/RETROSPECTIVE.md`. Default `false` (opt-in). Requires `gsd-core/workflows/retro.md` wiring (Phase 2). |

...
| `workflow.intermediate_bets_check` | boolean | `true` | Run an *intermediate-bets check* in `gsd-verifier` after terminal-outcome verification (Step 7d). Lists the necessary-but-not-sufficient gates each plan committed to and verifies hit/miss for each, separately from the terminal pass/fail. Non-blocking — surfaces decision-quality signal alongside outcome. Pairs with SDLC §16 v1.2.9 Release-Readiness Completion Comment (the comment is the receipt; this check reads the receipt against the actual record). Source: Duke/Rossi "Thinking in Bets for Engineers" (Refactoring S5 E8, 2025-08-01). |
| `workflow.provenance_footer` | boolean | `true` | Append a **Provenance** footer to agent-generated artifacts (`VERIFICATION.md`, `REVIEW.md`) naming source tier (verified · reviewed · raw · synthesized), data freshness (timestamp + HEAD sha), ownership (which agent at which depth/mode), and known unknowns (what the agent could not check). Doesn't make the answer more correct — lets the consumer judge how much to trust it. Defense-in-depth, not a silver bullet. Source: Anthropic, "How Anthropic enables self-service data analytics with Claude" (claude.com, 2026-06-03; raw#L202). |
| `workflow.adr_co_commit` | boolean | `true` | Enforce the ADR co-commit invariant in `gsd-verifier` (Step 7-ADR). ADRs live at `.planning/adrs/NNNN-kebab-case-title.md` with frontmatter (`id`, `title`, `status`, `date`, optional `supersedes` / `superseded-by`). Rules: (1) ADR-flagged tasks (`requires-adr: true` in PLAN.md frontmatter or `<adr-needed/>` marker) MUST ship a co-committed `.planning/adrs/NNNN-*.md`; missing ADR fails verification. (2) Supersede-don't-edit — existing ADRs are immutable except for `superseded-by` cross-references; modify-an-existing-ADR fails verification. Source: Tolaria AGENTS.md (Luca Rossi, 2026-04; raw#L46-L50). |
...
| `workflow.milestone_retro` | boolean | `false` | Enable multi-lens milestone retro. When `true`, `write_retrospective` invokes `gsd-core/workflows/retro.md` — spawning 4 isolated failure-class lenses + synthesizer — and injects a `### Tensions` subsection into the existing `.planning/RETROSPECTIVE.md` milestone entry (between `### Key Lessons` and `### Cost Observations`). Intermediate positions live in an ephemeral `.planning/tmp/retro-v{X.Y}/` scratch dir (deleted on success). `Candidate change:` lines in Tensions are the documented input to the next-transition `graduation` HITL. Default `false` (opt-in). |
| `workflow.teach_phase` | boolean | `false` | After verification, route each phase learning to the right durable surface (rules / skill / MCP tool / hook) via `gsd-core/workflows/teach-phase.md` and emit `NN-TEACH.md` HITL proposals only. Calls `gsd-extract-learnings` for candidate sourcing. Silent no-op when `false` or when zero candidates match a surface signal. Default `false` (opt-in). |
| `workflow.plan_lens_review` | boolean | `false` | At plan-phase, run 2–4 lens agents for advisory negotiated plan review before committing plans. Reuses frozen `gsd-lens` + `gsd-lens-synthesizer` agents (CON-lens-generic validation). Produces `NN-PLAN-RATIONALE.md` + `NN-KILL-CRITERIA.md`. Value-tradeoff conflicts escalate via AskUserQuestion; fact conflicts probe inline. Default `false` (opt-in). |

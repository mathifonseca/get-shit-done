...
The original interview-style flow. Claude identifies grey areas in the phase, presents them for selection, then asks all relevant questions per area until fully explored (fork default — `workflow.questions_per_area: "all"`; set to an integer like `4` for upstream's bounded behaviour). Good for:

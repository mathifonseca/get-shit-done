# Step: edge_case_hunter

Fork-added step (SDLC-aligned). Extracted from the execute-phase host loop per
ADR-857 phase 6 — the host loop body must stay under the pre-phase-6 byte
baseline, and optional feature logic belongs in a progressively-disclosed
fragment. Gated by `workflow.adversarial_validation`.

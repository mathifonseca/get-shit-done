...
const VALID_CONFIG_KEYS = new Set([
  ...
  'project.issue_tracker', 'project.issue_prefix',
  'project.pr_title_template', 'project.pr_body_requires_issue',
  'project.ci_commands',
]);
...
function buildNewProjectConfig(userChoices) {
  ...
  const hardcoded = {
    ...
    workflow: {
      ...
      skip_discuss: false,
      questions_per_area: 'all',
      test_contracts: true,
      adversarial_validation: true,
      dead_code_scan: true,
      playwright_verification: true,
      definition_of_done: true,
      preflight_on_verify: true,
      spec_outcome_enforcement: true,
      scaffold_makefile: true,
      scaffold_precommit: true,
      scaffold_preflight: true,
      scaffold_rules: true,
      update_claude_md_on_complete: true,
    },
    hooks: {

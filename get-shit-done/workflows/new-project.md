...
**Round 3 — Project Integration Settings:**

Use AskUserQuestion:
- header: "Tracking"
- question: "Where do you track issues for this project?"
- options: "GitHub Issues" / "Linear" / "Jira" / "None"

Store as `project.issue_tracker` (values: `github`, `linear`, `jira`, `null`).

If not "None":
  - header: "Prefix"
  - question: "What prefix do your issues use? (e.g., AXN, PROJ, or leave empty for no prefix)"
  - Record as `project.issue_prefix`

  - header: "Branches"
  - question: "What's your branch naming convention?"
  - options: 
    - "prefix-NN/description (e.g., axn-51/add-auth)" 
    - "feature/description"
    - "No convention"
  - If first option: store `git.branch_pattern` as `^[a-z]+-\\d+/`
  - If second: store `git.branch_pattern` as `^(feature|fix|refactor)/`

  - header: "PRs"
  - question: "Should PR titles follow a pattern?"
  - options:
    - "PREFIX-NN: Description (e.g., AXN-51: Add auth)"
    - "Conventional (feat: description)"
    - "No convention"
  - Store as `project.pr_title_template`

  - header: "PR Body"
  - question: "Require issue link in PR body? (Closes PREFIX-NN)"
  - options: "Yes" / "No"
  - Store as `project.pr_body_requires_issue`

  - header: "CI"
  - question: "What commands should pass before creating a PR?"
  - **Detect and suggest** based on what exists in the project:
    - If `Makefile` exists → suggest `["make check"]` (or `["make check", "make check-frontend"]` if frontend targets found)
    - If `package.json` exists with scripts → suggest based on available scripts:
      - `test` script → `npm test`
      - `lint` script → `npm run lint`
      - `typecheck` or `type-check` script → `npm run typecheck`
      - Combine into suggested array, e.g., `["npm run lint", "npm run typecheck", "npm test"]`
    - If `pyproject.toml` or `setup.py` exists → suggest `["make check"]` or `["pytest", "ruff check ."]`
    - If `Cargo.toml` exists → suggest `["cargo check", "cargo test", "cargo clippy"]`
    - If `go.mod` exists → suggest `["go vet ./...", "go test ./..."]`
    - If nothing detected → suggest `["make check"]` as the universal default
  - Present the suggestion as the default option, let user accept, modify, or provide their own
  - options: "{detected suggestion}" / "Custom (type your own)" / "None"
  - Store as `project.ci_commands` (array)

**Round 4 — Scaffolding:**

- header: "Makefile"
- question: "Scaffold a standard Makefile with setup/dev/check/lint/test targets?"
- options: "Yes" / "No"
- Store as `workflow.scaffold_makefile`

- header: "Pre-commit"
- question: "Scaffold pre-commit hook configuration?"
- options: "Yes" / "No"
- Store as `workflow.scaffold_precommit`

- header: "Preflight"
- question: "Scaffold a preflight checklist for pre-ship checks? (branch naming, tests, quality, secrets, docs)"
- options: "Yes" / "No"
- Store as `workflow.scaffold_preflight`

If "Yes":
  Detect the project stack and generate `.claude/preflight.yaml`:

  ```yaml
  # Auto-generated by GSD — customize for your project
  
  vcs:
    branch_pattern: "${git.branch_pattern or '^(feature|fix|refactor)/'}"
    protected_branches: ["main", "master"]
  
  testing:
    command: "${first ci_command or 'make check'}"
    require_new_tests: true
    coverage:
      enforce_non_decrease: true
  
  quality:
    ${if package.json: 'lint_command: "npm run lint"'}
    ${if package.json with typecheck script: 'typecheck_command: "npm run typecheck"'}
    ${if Makefile with lint target: 'lint_command: "make lint"'}
    ${if Makefile with typecheck target: 'typecheck_command: "make typecheck"'}
    forbidden_patterns:
      - pattern: "console\\.log"
  ...
  dependencies:
    ...
    lockfiles:
      ...
      ${if pyproject.toml: '- "uv.lock"  # or poetry.lock, requirements.txt'}
      ${if Cargo.toml: '- "Cargo.lock"'}
      ${if go.mod: '- "go.sum"'}
  
  documentation:
    require_updates:
      ${if src/api or src/app/api exists:
      - when: "src/api/**"
        path: "docs/"
        message: "API files changed — update docs"}
  
  pr_hygiene:
    max_files_changed: 50
  ```

  Adapt the YAML based on what actually exists in the project (don't include sections for tools that aren't present). Write to `.claude/preflight.yaml`.

  Commit the scaffolded file:

  ```bash
  node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" commit "chore: scaffold preflight checklist" --files .claude/preflight.yaml
  ```

- header: "Rules"
- question: "Scaffold .claude/rules/ with domain-specific rule files?"
- options: "Yes" / "No"
- Store as `workflow.scaffold_rules`

If "Yes":
  Detect project structure and create rule files. Each file has `paths:` frontmatter controlling when it loads.

  **Always create:**
  - `.claude/rules/principles.md` — Design principles (from SDLC: DB constraints, trust boundaries, ratchet effect, dead code is pollution)
    ```yaml
    ---
    paths:
      - "src/**"
  ...
  - `.claude/rules/testing.md` — Test conventions (framework, fixture patterns, quality gate commands)
    ```yaml
    ---
    paths:
      - "tests/**"
  ...
  **Create if detected:**
  - `.claude/rules/api.md` if `src/api/` or `src/app/api/` or `routes/` exists — API route patterns, response models
    ```yaml
    ---
    paths:
      - "src/api/**"
  ...
  - `.claude/rules/frontend.md` if `src/components/` or `src/app/` (Next.js) or `src/pages/` exists — Component patterns, state management
    ```yaml
    ---
    paths:
      - "src/components/**"
  ...
  - `.claude/rules/db.md` if `prisma/` or `migrations/` or `alembic/` exists — ORM models, migration conventions
    ```yaml
    ---
    paths:
      - "prisma/**"
  ...
  - `.claude/rules/infra.md` if `Dockerfile` or `.github/workflows/` or `docker-compose.yml` exists — Build config, CI, Docker
    ```yaml
    ---
    paths:
      - "Dockerfile*"
...
```bash
mkdir -p .planning
node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-new-project '{"mode":"[yolo|interactive]","granularity":"[selected]","parallelization":true|false,"commit_docs":true|false,"model_profile":"quality|balanced|budget|inherit","workflow":{"research":true|false,"plan_check":true|false,"verifier":true|false,"nyquist_validation":[false if granularity=coarse, true otherwise],"scaffold_makefile":true|false,"scaffold_precommit":true|false,"scaffold_preflight":true|false,"scaffold_rules":true|false},"project":{"issue_tracker":"github|linear|jira|null","issue_prefix":"[selected or null]","pr_title_template":"[selected or null]","pr_body_requires_issue":true|false,"ci_commands":["[user input]"]},"git":{"branch_pattern":"[selected or null]"},"ship":{"pr_body_sections":[{"heading":"User Stories & Acceptance Criteria","enabled":true|false,"source":"REQUIREMENTS.md ## User Stories || REQUIREMENTS.md ## Acceptance Criteria","fallback":"- Acceptance criteria are covered by the linked requirements and verification evidence."},{"heading":"Risks & Dependencies","enabled":true|false,"source":"PLAN.md ## Risks || PLAN.md ## Dependencies","fallback":"- No known high-risk rollout dependencies."},{"heading":"Success Metrics & Release Criteria","enabled":true|false,"source":"REQUIREMENTS.md ## Definition of Done || VERIFICATION.md ## Release Criteria","fallback":"- Release when automated verification and required manual checks pass."},{"heading":"Stakeholder Review & Approval","enabled":true|false,"template":"- Product owner approval pending for {phase_name}."}]}}'
```
...
- `.claude/preflight.yaml` (if preflight scaffolding selected)
- `.claude/rules/` (if rules scaffolding selected)
  - `principles.md`
  - `testing.md`
  - `api.md` (if API routes detected)
  - `frontend.md` (if frontend components detected)
  - `db.md` (if database layer detected)
  - `infra.md` (if infrastructure files detected)

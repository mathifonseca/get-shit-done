...
<step name="adversarial_validation">
**Adversarial Validation Gate** (optional — runs when `workflow.adversarial_validation` is enabled)

```bash
ADVERSARIAL=$(node "$HOME/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.adversarial_validation 2>/dev/null || echo "false")
```

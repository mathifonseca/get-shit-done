  ...
  try {
    ...
    if (fs.existsSync(cacheFile)) {
      try {
        ...
        // Fork-aware path (fork-specific): when worker reports is_fork=true, show the
        // "cherry-pick from upstream" cyan banner instead of the standard /gsd:update
        // amber, and suppress stale-hook warnings since the fork's rebuild flow manages
        // hooks separately (npm run build:hooks → node bin/install.js).
        if (cache.is_fork) {
